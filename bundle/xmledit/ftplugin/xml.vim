...
function s:ParseTag( )
    ...
    if (&filetype == 'html' || &filetype == 'xhtml') && (!exists ("g:xml_no_html"))
        ...
        let ltag = substitute (ltag, '<\s*\([^[:alnum:][=a=][=c=][=e=][=i=][=o=][=u=]_:\-[:blank:]]\=\)\s*\([[:alnum:][=e=]_:\-]\+\)\>', '<\1\2', '')
    else
        let html_mode = 0
    endif
    ...
    if <SID>IsParsableTag (ltag)
        ...
        "
        let index = matchend (ltag, '[[:alnum:][=a=][=c=][=e=][=i=][=o=][=u=]_:\.\-]\+')
        "let index2 = matchend (ltag, '[[:alnum:]_:\.\-]\+')
        "let index2 = substitute(ltag,'[[=a=][=c=][=e=][=i=][=o=][=u=]]','e','g')
        "execute "normal! A \<CR>ltag: " . ltag . " ltag2: " . ltag2 . " i2: " . index2 . " i1:" . index " \<CR>\<ESC>"
        ...
        if html_mode && tag_name =~? '^\(img\|input\|param\|frame\|br\|hr\|meta\|link\|base\|area\)$'
            ...
            if multi_line
                ...
                let ltag2 = substitute(ltag,'[[=a=][=c=][=e=][=i=][=o=][=u=]]','a','g')
                let index = matchend (ltag2, '[[:alnum:][=a=][=c=][=e=][=i=][=o=][=u=]_:\.\-]\+')+3
...
function s:TagMatch1()
 ...
 if match(b:xreg,'^[[:alnum:][=a=][=c=][=e=][=i=][=o=][=u=]_:\-]') != -1
